...
- readonly()的确修饰了一个响应式数据car,但是它的返回值仍然是一个代理的对象(也就是说readonly处理了一个响应式的数据，但是并没有把他变成object数据，而是依然是Proxy这个代理对象)
```js
  setup(){
    let car = reactive({name:'奔驰',price:'40W'})
    let sum = ref(0)
    ...
    console.log(isProxy(sum)) //false,因为这里使用的是getter和setter,而不是Proxy
...
实际上必须借助`hook`,这样子才能把相关功能的代码组合起来并组件化，按照功能点进行打包
...
- 什么是Teleport？—— `Teleport` 是一种能够将我们的<strong style="color:#DD5145">组件html结构</strong>移动到指定位置的技术。
  ...
  ```vue
  <teleport to="移动位置"> //把要传送走的结构放在teleport标签中，移动位置可以写body,html，css选择器等任意位置
...
- 使用步骤：
  ...
  - 异步引入组件
    ...
    //以前的静态引入是这样的：
    import Child from './components/Child'  //只能一起出现
    ```
  ...
  - 使用```Suspense```包裹组件，并配置好```default``` 与 ```fallback```
    ...
    <template>
    	<div class="app">
    		<h3>我是App组件</h3>
    		<Suspense> //不需要引入，是内置的，本质上是使用插槽slot实现的
    			<template v-slot:default>  //default插槽放置真正应该要展示的东西
    				<Child/>  //要把真正要展示的东西放在插槽里，并且插槽要有名字
    			</template>
    			<template v-slot:fallback> //fallback插槽放置
    				<h3>加载中.....</h3>
    			</template>
    		</Suspense>
    	</div>
...
- <strong style="color:#DD5145">移除</strong>```v-on.native```修饰符
  ...
  - 父组件中绑定事件（绑定自定义事件）
    ...
    ```vue
    <my-component
      v-on:close="handleComponentEvent" //定义一个自定义事件close
  ...
  - 子组件中声明自定义事件
    ...
    <script>
      export default {
        emits: ['close'] //用emits声明的就是自定义事件，不写的话默认原生事件
      }
    </script>
    ```
